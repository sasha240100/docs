...
export async function HomePage() {
    ...
    if (userInfoResponse.status === 200) {
        ...
        // SessionAuth will redirect based on which claim is invalid
        return <SessionAuth />;
    }
    ...
    // Get initial session auth context (SSRSessionAuthContext) 
    // and pass session data to the client
...
We then use `getInitialSessionAuthContext(session)` to generate `initialSessionAuthContext` and pass it to `SessionAuth` (client component). This session context will be used on server render and the first render before mount on the client side. For detailed explanation [see this issue](https://github.com/supertokens/supertokens-auth-react/issues/790).
